...
def stacking_regression(models, meta_model, X_train, y_train, X_test,
    ...
    kf = KFold(n_splits = n_folds, shuffle = shuffle, random_state = random_state) #provides indices

    if X_train.__class__.__name__ == "DataFrame":
    	X_train = X_train.as_matrix() #convert X_train to matrix
    	X_test = X_test.as_matrix() #convert X_test to matrix

    # Create empty numpy arrays for stacking features
    S_train = np.zeros((X_train.shape[0], len(models))) #S_train is a 1458 x 3 matrix of zeroes
    S_test = np.zeros((X_test.shape[0], len(models))) #S_test is a 1459 x 3 matrix of zeroes
    ...
    for model_counter, model in enumerate(models):
        ...
        for fold_counter, (tr_index, te_index) in enumerate(kf.split(X_train, y_train)):
            ...
            instance = clone(model) #clone EN
            # Fit 1-st level model
            instance.fit(X_tr, transformer(y_tr, func = transform_target)) #Fit ElasticNet in fold
            # Predict out-of-fold part of train set
            S_train[te_index, model_counter] = transformer(instance.predict(X_te), func = transform_pred) #predict OOF
    ...
    RandomForestRegressor(n_estimators = 2500, min_samples_split = 2, max_features = 'sqrt', random_state = 1)
    make_pipeline(RobustScaler(), ElasticNet(alpha = 0.0016, l1_ratio = 0.2, random_state = 1)),
    GradientBoostingRegressor(n_estimators = 940, learning_rate = 0.025, max_depth = 3, min_samples_split = 35, min_samples_leaf = 3, max_features = 13, 
    	subsample = 0.8, loss = 'huber', random_state = 1)
